...
<template>
    <div class="animate-fade z-10 flex-col m-auto mb-12">
        <div class="w-full mr-8">
            <h1 class="font-title text-2xl font-bold">{{props.title}}</h1>
            <br>
            <h1 class="font-alt text-gray-800 text-sm">{{props.shortDesc}}</h1>
            <div v-if="props.longDesc && !activeModal" class="relative group inline-flex pt-2 pb-2 pr-2 h-4 mt-2 text-sm 
                ...
                <svg v-if="!activeModal"
                    class="fill-gray-500 rotate-90 h-4 w-4 group-hover:fill-white transition-all duration-200"
                ...
                <div v-if="!activeModal" class="font-alt">Learn more</div>
            </div>
            <div 
                v-if="activeModal" 
                class="w-full h-6 flex justify-end mt-2 mb-2"
                @click="activeModal = false"
                >
                <CloseButton style-string="h-5 " />
            </div>
            <div v-if="activeModal" class="w-full font-alt text-sm text-gray-800" v-html="props.longDesc"></div>
            <h2 class="font-alt font-light mt-4 mb-2">Technology Stack</h2>
            <div class="w-full flex flex-wrap">
                <img class="h-6 mr-2 mb-4" v-for="item in props.tech" :src="'/svg/'+item+'.svg'" :key="uuidv4()">
            </div>
        </div>
        <div class="relative">
            ...
            <a :href="props.githubUrl" target="_blank" rel="noopener noreferrer">
                <button
                    class="absolute group bottom-2 left-2 sm:bottom-4 sm:left-4 drop-shadow-md font-alt font-light 
                            flex items-center justify-center bg-white h-8 w-30 sm:h-14 sm:w-48 p-3 
                            text-gray-800 hover:bg-mainPurple hover:text-white transistion-all duration-200 text-xs sm:text-lg">
            ...
            <a :href="props.liveUrl" target="_blank" rel="noopener noreferrer">
                <button
                    class="absolute group bottom-2 right-2 sm:bottom-4 sm:right-4 drop-shadow-md font-alt font-light 
                            flex items-center justify-center bg-white h-8 w-30 sm:h-14 sm:w-48 p-3 
                            text-gray-800 hover:bg-mainPurple hover:text-white transistion-all duration-200 text-xs sm:text-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 mr-4 group-hover:stroke-white stroke-black"
                        viewBox="0 0 24 24" stroke-width="2" stroke="#2c3e50" fill="none" stroke-linecap="round"
